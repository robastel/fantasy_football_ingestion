...
def get_logger(name, stream=sys.stdout, style='{', frmt='{name} [{levelname}]:: {message}', level='INFO'):
    """
    Create a logger.
    If the logger already exists, clear the handlers and re-add them according to the parameters provided.
    :param name: The name ofthe logger
...
def format_response(response, key_map):
    """
    Takes a nested dictionary (a record), or list of (potentially nested) dictionaries (records); and filters, flattens,
    and renames the key-value pairs of each record according to another dictionary (key map) provided by the user.
    :param response: An API response of a single record (a dictionary) or multiple records (a list of dictionaries)
    :param key_map: A (potentially nested) dictionary used to filter and flatten each record in the response.
    :return: Same type as response; with each record filtered, flattened, and renamed according to the key_map
    """
...
def _format_record(record, key_map):
    """
    Helper function to format a single record as described in format_response
    :param record: A (potentially nested) dictionary representing a single record from an API response
    :param key_map: A (potentially nested) dictionary used to filter and flatten the record
    :return: A dictionary; filtered, flattened, and renamed according to the key_map
    """
...
def _format_key(record, key, value, formatted_record):
    """
    A recursive helper function to handle nested dictionaries from _format_record
    :param record: The record received from _format_record
    :param key: A key in the key_map from _format_record
    :param value: The corresponding value in the key_map from _format_record
    :param formatted_record: A dictionary; filtered, flattened, and renamed according to the key_map from _format_record
    :return: None
    """
